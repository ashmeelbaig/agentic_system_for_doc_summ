...
def print_model_comparison(model_results: Dict[str, Dict[str, Any]]) -> None:
    ...
    for model_id, result in model_results.items():
        ...
        if result.get("status") == "success":
            ...
            print(f"Safety action: {safety.get('action', 'N/A')}")
            print(f"Refusal: {str(result.get('is_refusal_answer', False)).lower()}")
        else:
            print("Final answer: N/A")
            print("Faithfulness score: N/A")
            print("Revision decision: N/A")
            print("Safety action: N/A")
            print("Refusal: N/A")
            print(f"Error: {result.get('error', 'Model generation failed.')}")
        print(f"Latency: {float(result.get('latency_seconds', 0.0)):.2f}s")
        print("")
